...
export default function AdsTxtPage() {
  ...
  const { data: publishersData, refetch: refetchPublishers } = useQuery<{
    ...
    queryKey: ['ads-txt-publishers'],
    queryFn: async () => {
      const res = await fetch('/api/ads-txt/publishers');
  ...
  const { data: historyData, refetch: refetchHistory } = useQuery<{
    ...
    queryKey: ['ads-txt-history'],
    queryFn: async () => {
      const res = await fetch('/api/ads-txt/history');
      if (!res.ok) throw new Error('Failed');
      return res.json();
    },
  });
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/ads-txt/search', {
        method: 'POST',
  ...
  const handleDeletePublisher = async (id: number) => {
    try {
      const res = await fetch(`/api/ads-txt/publishers?id=${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  const handleClearHistory = async () => {
    try {
      const res = await fetch('/api/ads-txt/history', { method: 'DELETE' });
      if (res.ok) {
  ...
  const handleExportJson = () => {
    ...
    a.href = url;
    a.download = `ads-txt-results-${Date.now()}.json`;
    a.click();
  ...
  return (
    <Box>
      <PageHeader title="Ads.txt Tracker" />
      ...
      >
        <Box sx={{ display: 'flex', gap: 1.5, mb: isSearching ? 2 : 0 }}>
          <TextField
            fullWidth
            size="small"
            placeholder='e.g. "google.com", "pub-1234567890", "DIRECT"'
            value={searchLine}
