...
import { DEFAULT_PUBLISHERS_WEB, extractDomain } from '@/lib/ads-txt/default-publishers';
...
export async function GET() {
  try {
    ...
    console.error('[Ads-Txt Publishers] GET error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === 'reset_defaults') {
      ...
      if (DEFAULT_PUBLISHERS_WEB.length > 0) {
        const rows = DEFAULT_PUBLISHERS_WEB.map((url) => ({
      ...
      return NextResponse.json({
        success: true,
        message: `Reset to ${DEFAULT_PUBLISHERS_WEB.length} default publishers`,
        count: DEFAULT_PUBLISHERS_WEB.length,
      });
    }
    ...
    if (body.urls && Array.isArray(body.urls)) {
      const rows = body.urls
        .map((url: string) => {
          ...
          if (!normalized.endsWith('/ads.txt')) {
            normalized = normalized.replace(/\/?$/, '/ads.txt');
          }
          return {
    ...
    console.error('[Ads-Txt Publishers] POST error:', error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    let query = supabase.from('publisher_domains_web').delete();
    ...
    console.error('[Ads-Txt Publishers] DELETE error:', error);
    return NextResponse.json(
